...
function validateInput(name, courses) {
  if (
    (name === null || name === void 0 ? void 0 : name.trim()) === "" ||
    name === undefined
  )
    return false;
  if (isNaN(Number(courses))) return false;
  return true;
}
function calculatePrice(courses) {
  if (courses <= 10) return 1500;
  if (courses <= 20) return 1300;
  return 1100;
}
function addPurchaseRecord(name, courses) {
  if (validateInput(name, courses)) {
    let price = calculatePrice(courses);
    const record = {
      name: name,
      courses: courses,
      price: price,
    ...
    console.log(
      `新增購買記錄成功！會員 ${name} 購買 ${courses} 堂課，總金額為 ${record.total} 元`
    );
  } else console.log("輸入錯誤，請輸入有效的會員名稱和課程數量");
}
...
function calculateTotalPrice() {
  let totalPrice = purchaseRecords.reduce(
...
function filterNoPurchaseMember() {
  const recordMember = [
    ...new Set(purchaseRecords.map((record) => record.name)),
  ];
  const noPurchaseMember = members.filter(
    (member) => recordMember.indexOf(member) === -1
  );
